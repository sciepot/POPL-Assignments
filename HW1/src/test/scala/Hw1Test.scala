class Hw1Test extends org.scalatest.funsuite.AnyFunSuite {
  ...
  test("Hw1.insert") {
    ...
    val step_3 = IntNode(10, Leaf, Leaf)
    val step_4 = Hw1.insert(step_3, 10)
    val step_5 = Hw1.insert(step_3, 7)
    val step_6 = Hw1.insert(step_5, 8)
    assert(step_4 === IntNode(10, Leaf, Leaf))
    assert(step_5 === IntNode(10,IntNode(7, Leaf, Leaf), Leaf))
    assert(step_6 === IntNode(10,IntNode(7, Leaf, IntNode(8, Leaf, Leaf)), Leaf))
  }

  test("Hw1.eval") {
    assert(Hw1.eval(True) === true)
    assert(Hw1.eval(Andalso(Orelse(True,False),True)) === true)
    assert(Hw1.eval(Implies(False, False)) === true)
  }
